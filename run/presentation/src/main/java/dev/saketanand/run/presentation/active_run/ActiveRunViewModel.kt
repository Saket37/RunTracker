...
class ActiveRunViewModel(
    ...
    init {
        hasLocationPermission.onEach { hasPermission ->
            if (hasPermission) runningTracker.startObservingLocation() else runningTracker.stopObservingLocation()
    ...
    fun onAction(action: ActiveRunAction) {
        when (action) {
            ...
            ActiveRunAction.OnToggleRunClick -> {
                state = state.copy(hasStartedRunning = true, shouldTrack = !state.shouldTrack)
            }

            is ActiveRunAction.SubmitLocationPermission -> {
            ...
            is ActiveRunAction.SubmitNotificationPermission -> {
                state = state.copy(
